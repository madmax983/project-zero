...
### INT-1051: Asteroid Claims Attack -> Chronicle
- **Date:** 2026-06-28
- **Systems connected:** `AttackColonyEvent` -> `attack_colony_chronicle_bridge` -> `AddChronicleEvent`
- **Glue added:** `attack_colony_chronicle_bridge` in `src/layer2/integration.rs` translates the `AttackColonyEvent` generated by a hostile faction using asteroid royalties into a historical `AddChronicleEvent` for the colony to remember. Registered in `src/simulation.rs`.
- **Tests:** `tests/integration/asteroid_claim_attack.rs`

### INT-1035: Modular Fauna Stats -> Economy
- **Date:** 2026-10-31
