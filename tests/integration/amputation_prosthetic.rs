...
fn test_amputation_applies_component_and_memory() {
    ...
    let pop = world
        .spawn((Pop, Memories::default(), Augmentations::default()))
        .id();
    ...
    assert!(
        memories
            .items
            .iter()
            .any(|m| m.memory_type == MemoryType::LostLimb),
        "Pop should have LostLimb memory"
    );
}
...
fn test_missing_limb_penalty() {
    ...
    let pop = world
        .spawn((Pop, Augmentations::default(), MissingLimb { severity: 0.5 }))
        .id();
...
fn test_prosthetic_surgery_removes_missing_limb() {
    ...
    assert!(
        world.get::<MissingLimb>(pop).is_none(),
        "MissingLimb should be removed after surgery"
    );
