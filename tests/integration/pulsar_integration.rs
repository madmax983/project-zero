...
use scale::layer2::pulsar::{Pulsar, Facing, pulsar_rotation_system, pulsar_radiation_damage_system};
...
fn test_pulsar_integration_wiring() {
    ...
    app.insert_resource(SimulationTime { tick: 10, ..Default::default() });

    app.add_systems(bevy_app::Update, (
        pulsar_rotation_system,
        pulsar_radiation_damage_system,
    ).chain());

    let _pulsar_id = app.world_mut().spawn((
        Pulsar {
