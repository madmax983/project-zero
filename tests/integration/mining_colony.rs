...
use scale::layer2::mining::{FleetCargo, CargoStack};
...
fn test_fleet_unloads_cargo_at_colony() {
    ...
    assert!((resources.ore - 10.0).abs() < f32::EPSILON, "Colony should receive 10.0 Ore, but has {}", resources.ore);

    // Assert: Fleet is empty
    let cargo = world.get::<FleetCargo>(fleet).unwrap();
    assert!(cargo.current_load() < f32::EPSILON, "Fleet cargo should be empty, but has {}", cargo.current_load());
}
...
fn test_fleet_does_not_unload_elsewhere() {
    ...
    let resources = world.resource::<ColonyResources>();
    assert!(resources.ore < f32::EPSILON, "Colony should not receive Ore from remote fleet");

    // Assert: Fleet still has cargo
    let cargo = world.get::<FleetCargo>(fleet).unwrap();
    assert!((cargo.current_load() - 10.0).abs() < f32::EPSILON, "Fleet should keep cargo");
}
