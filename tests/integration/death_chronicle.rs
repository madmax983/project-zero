...
use scale::layer1::health::{death_system, Health};
...
fn test_pop_death_adds_chronicle_entry() {
    ...
    world.insert_resource(ColonyName { name: "Test Colony".to_string() });
    ...
    let pop = world.spawn((
        ...
        Health { current: 0.0, max: 100.0 }, // Dead
    )).id();
    ...
    assert!(!emitted.is_empty(), "Death should trigger a chronicle event");
    assert!(emitted[0].text.contains("TestSubject"), "Event text should contain pop name: {}", emitted[0].text);
}
