...
use bevy_app::App;
use scale::layer1::shadow_ecosystems::{EmMachine, ShortCircuitEvent, shadow_ecosystems_short_circuit_bridge};
...
fn test_shadow_ecosystems_short_circuit_bridge() {
    ...
    app.world_mut()
        ...
        .send(ShortCircuitEvent { target: machine_entity });

    app.update();

    // Verify the machine is now inactive
    let machine = app.world().get::<EmMachine>(machine_entity).unwrap();
    assert!(!machine.active, "EmMachine should be deactivated by the short circuit bridge");
