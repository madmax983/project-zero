...
fn test_founder_effect_bridge_system() {
    ...
    let pop_entity = app.world_mut().spawn((
        Pop,
        Traits(bevy::utils::HashSet::new()),
    )).id();

    // Fire the PopBorn event
    app.world_mut().resource_mut::<Events<PopBorn>>().send(PopBorn {
        entity: pop_entity,
    ...
    assert!(pop_traits.has(Trait::StoneSkin), "Pop should have acquired the dominant trait");
}
