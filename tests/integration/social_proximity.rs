...
fn test_proximity_social_buff_applied() {
    ...
    let pop2 = world
        .spawn((Pop, GridPosition { x: 10, y: 11 }, Relationships::default()))
        .id();
    ...
    assert!(
        buff2.is_some(),
        "Pop 2 should have SocialBuff from friend nearby"
    );
    assert!(
        buff2.unwrap().value > 0.0,
        "Pop 2 should have positive buff"
    );
}
...
fn test_proximity_social_debuff_applied() {
    ...
    assert!(
        buff1.is_some(),
        "Pop 1 should have SocialBuff from enemy nearby"
    );
    assert!(
        buff1.unwrap().value < 0.0,
        "Pop 1 should have negative buff"
    );
}
