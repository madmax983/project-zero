...
fn test_haunted_assembly_lines_basic_behavior() {
    ...
    app.add_systems(bevy_app::Update, (haunted_building_system, apply_haunted_stress_system));

    // Setup a factory and a worker
    let factory = app.world_mut().spawn((Building { building_type: BuildingType::LumberMill }, Efficiency(1.0))).id();
    let pop = app.world_mut().spawn((Pop, Stress(0.0), AssignedTo { entity: factory, assignment_type: AssignmentType::FarmWorker })).id();
    ...
    let new_worker = app.world_mut().spawn((Pop, Stress(0.0), AssignedTo { entity: factory, assignment_type: AssignmentType::FarmWorker })).id();
...
fn test_haunted_assembly_lines_edge_cases() {
    ...
    let mut app = App::new();
    app.add_event::<PopDiedInAccidentEvent>();
    app.add_systems(bevy_app::Update, (haunted_building_system, apply_haunted_stress_system, check_haunted_worker_system));

    // Setup an already haunted factory
    let factory = app.world_mut().spawn((Building { building_type: BuildingType::LumberMill }, Efficiency(1.5), EchoOfTheFallen)).id();
    let pop = app.world_mut().spawn((Pop, Stress(0.0), AssignedTo { entity: factory, assignment_type: AssignmentType::FarmWorker })).id();
    ...
    let stressed_worker = app.world_mut().spawn((Pop, Stress(1.0), AssignedTo { entity: factory, assignment_type: AssignmentType::FarmWorker })).id();
    app.update();
