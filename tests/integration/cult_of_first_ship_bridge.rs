...
fn first_ship_destruction_causes_unrest_event() {
    ...
    app.add_systems(Update, (first_ship_destruction_system, first_ship_destruction_chronicle_bridge).chain());
    ...
    assert!(events_list[0].text.contains("First Ship"), "Event text should mention First Ship");
}
