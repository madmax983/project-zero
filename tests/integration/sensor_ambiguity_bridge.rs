...
use scale::layer2::integration::{
    assign_sensors_to_player_fleets_system, ensure_player_fleets_identified_system,
};
use scale::layer2::sensor_ambiguity::{
    resolve_sensors_system, SensorContact, Sensors, UnidentifiedContact,
};
...
fn player_fleets_have_sensors_and_are_identified() {
    ...
    let player_fleet = app
        .world_mut()
        .spawn((Fleet, FleetFaction::Player, InOrbit { parent: planet }))
        .id();

    // Spawn Pirate Fleet B far away (should be unidentified)
    let pirate_fleet = app
        .world_mut()
        .spawn((
            Fleet,
            FleetFaction::Pirate,
            InOrbit {
                parent: faraway_planet,
            },
        ))
        .id();
    ...
    assert!(
        app.world()
            .get::<UnidentifiedContact>(player_fleet)
            .is_none(),
        "Player Fleet should never be unidentified"
    );

    let contact_player = app
        .world()
        .get::<SensorContact>(player_fleet)
        .expect("Player Fleet should have SensorContact");
    assert_eq!(
    ...
    assert!(
        app.world()
            .get::<UnidentifiedContact>(pirate_fleet)
            .is_some(),
        "Faraway Pirate Fleet should be unidentified"
    );

    let contact_pirate = app
        .world()
        .get::<SensorContact>(pirate_fleet)
        .expect("Pirate Fleet should have SensorContact");
    assert!(
