...
fn test_paranoia_triggers_stress_breakdown() {
    ...
    app.add_systems(
        Update,
        (
            paranoia_stress_bridge_system,
            check_stress_breakdown_system,
        )
            .chain(),
    );
    ...
    assert_eq!(stress.accumulated_stress, 0.0, "Stress should be reset after breakdown");
}
...
fn test_paranoia_adds_to_stress_without_breakdown() {
    ...
    let stress = app.world().get::<StressTracker>(pop).unwrap();
    assert_eq!(stress.accumulated_stress, 30.0, "Stress should accumulate paranoia");
