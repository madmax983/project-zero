...
use scale::layer1::economy::existential_audit::{
    existential_audit_system, existential_crisis_decay_system, ExistentialCrisis,
    IndustrialBuilding, PrecursorAI,
};
...
fn test_existential_audit_integration() {
    ...
    app.add_systems(
        Update,
        (existential_audit_system, existential_crisis_decay_system).chain(),
    );
    ...
    assert!(
        ...
        "Pop should have crisis after failed audit"
    );
    ...
    assert!(
        !pop.contains::<ExistentialCrisis>(),
        "Crisis should decay after 500 updates"
    );
}
