...
mod tests {
    use bevy::prelude::*;
    use scale::layer1::culture::celestial_cemeteries::{OrbitalCemetery, LaunchSequence, calculate_launch_risk_system};


    use scale::layer2::trade::escape_velocity::{TradeManifest, LaunchShipEvent, process_launch_system, CargoItem};
    ...
    fn test_celestial_cemeteries_launch_risk_bridge() {
        ...
        app.add_plugins(MinimalPlugins)
            ...
            .insert_resource(ColonyResources { fuel: 2000.0, ..Default::default() });

        app.add_event::<LaunchShipEvent>();


        app.add_systems(Update, (
            calculate_launch_risk_system,
        ...
        let manifest_entity = app.world_mut().spawn((
            TradeManifest {
        ...
        let risk = app.world().get::<LaunchSequence>(manifest_entity).unwrap().base_risk;
        assert!(risk > 0.05, "Launch risk should be increased due to celestial cemetery coffins");
