...
fn test_orbital_drydock_fleet_bridge() {
    ...
    let drydock_entity = app.world_mut().spawn(Station {
        station_type: StationType::OrbitalDrydock,
    }).id();

    app.world_mut().resource_mut::<Events<ShipConstructionCompletedEvent>>().send(
        ShipConstructionCompletedEvent {
            drydock_entity,
    ...
    assert!(fleet_spawned, "Fleet should be spawned with InOrbit attached.");
