...
fn test_trade_route_ideological_contraband_bridge() {
    ...
    let pop_entity = app
        .world_mut()
        .spawn((Pop, Ethics { collectivism: 0, elitism: 0 }))
        .id();

    let colony_a = app.world_mut().spawn(Colony {
        name: "Earth".to_string(),
        resources: vec![("Worker Boots".to_string(), 500)],
    }).id();

    let colony_b = app.world_mut().spawn((Colony {
        name: "Mars".to_string(),
        resources: vec![],
    }, HomeColony)).id();

    app.world_mut().spawn((TradeRoute {
        source: colony_a,
    ...
    assert!(ethics.collectivism > 0, "Collectivism should increase due to the imported Worker Boots");
}
