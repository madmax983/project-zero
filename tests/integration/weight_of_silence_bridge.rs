...
fn test_trade_route_resets_colony_isolation() {
    ...
    app.insert_resource(scale::shared::time::SimulationTime { tick: 1000, ..Default::default() });

    app.add_systems(Update, reset_isolation_on_trade_system);

    let colony_ent = app.world_mut().spawn(ColonyNode {
...
fn test_silence_cult_chronicle_bridge() {
    ...
    app.world_mut().spawn(SilenceCult { colony_entity: Entity::PLACEHOLDER });
