...
use scale::layer3::bureaucracy_of_vanity::{
    vanity_building_listener_system, ActiveDemands, ImperialStanding, VainGovernor, VanityProject,
    vanity_sabotage_system, GlobalEfficiency,
};
...
fn test_vanity_integration() {
    ...
    assert!(eff < 1.0, "Efficiency should have dropped due to ignored demand.");
    ...
    assert_eq!(standing, 60, "Imperial Standing should increase upon building completion.");

    // Demand is no longer active
    assert!(!app.world().resource::<ActiveDemands>().vanity_demand_active, "Demand should be satisfied.");
}