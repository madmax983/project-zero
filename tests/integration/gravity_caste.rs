...
mod integration_tests {
    ...
    fn test_gravity_caste_seam() {
        ...
        app.add_systems(
            Update,
            (adapt_gravity_caste_system, apply_gravity_penalties_system).chain(),
        );
        ...
        assert!(
            speed.current < 5.0,
            "System apply_gravity_penalties_system did not run, speed unaffected"
        );
        assert!(
            health.current < 100.0,
            "System apply_gravity_penalties_system did not run, health unaffected"
        );
    }
}
