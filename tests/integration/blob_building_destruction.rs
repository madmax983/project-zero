...
use scale::layer1::building::{Building, BuildingMap, BuildingType};
use scale::layer1::entities::blob::blob_building_destruction_system;
use scale::layer1::entities::blob::{blob_expansion_system, BlobNetwork, BlobNode};
...
fn test_blob_destroys_building() {
    ...
    // Provide BuildingMap
    let mut building_map = bevy::utils::HashMap::default();

    app.add_systems(
        Update,
        (blob_expansion_system, blob_building_destruction_system).chain(),
    );
    ...
    let event_reader: Vec<_> = cursor.read(events).collect();

    assert_eq!(
        event_reader.len(),
        1,
        "Blob should destroy the building and emit an event"
    );
