...
mod tests {
    ...
    fn test_defaulting_on_loan_triggers_repo_drones() {
        ...
        assert!(
            ...
            "Loan default should spawn repo drones in orbit"
        );
    }
    ...
    fn test_repo_drones_remove_high_value_buildings_from_layer_1() {
        ...
        assert!(
            app.world().get::<BuildingValue>(hospital).is_none(),
            "Repo drone should extract (despawn) the high-value building"
        );
    }
    ...
    fn test_repo_drones_reduce_colony_debt_upon_extraction() {
        ...
        let generator = app
            .world_mut()
            .spawn(BuildingValue { value: 40_000.0 })
            .id();
        ...
        assert_eq!(
            finances.debt, 60_000.0,
            "Extracting a building should reduce the debt by its value"
        );
    }
}
