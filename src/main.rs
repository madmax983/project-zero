...
use scale::layer1::{
    ...
    OccupiedTiles, RenderCache, Viewport, check_milestones_system,
    clean_dead_residents_system, clean_dead_workers_system, consume_food_system,
    decay_needs_system, generate_terrain, initial_chronicle_event, kill_starving_entities_system,
    produce_food_system, restore_rest_in_housing_system, spawn_initial_pops, update_render_cache,
...
fn run_app(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>) -> anyhow::Result<()> {
    ...
    loop {
        ...
        // Render
        terminal.draw(|frame| scale::ui::draw_ui(frame, &world))?;
    }
