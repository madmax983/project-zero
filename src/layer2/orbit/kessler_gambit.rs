...
mod tests {
    ...
    fn test_trigger_kessler_gambit_destroys_infrastructure() {
        ...
        let station1 = app.world_mut().spawn((Station { station_type: StationType::Outpost }, InOrbit { parent: planet1 })).id();
        let station2 = app.world_mut().spawn((Station { station_type: StationType::MiningPlatform }, InOrbit { parent: planet1 })).id();
        let fleet = app.world_mut().spawn((Fleet, InOrbit { parent: planet1 })).id();

        // This one should survive as it's around a different planet
        let station3 = app.world_mut().spawn((Station { station_type: StationType::Outpost }, InOrbit { parent: planet2 })).id();

        // Act
        app.add_systems(Update, trigger_kessler_gambit_system);
        app.world_mut().send_event(TriggerKesslerGambitEvent { planet: planet1 });
        app.update();

        // Assert
        assert!(app.world().get::<Station>(station1).is_none(), "Station 1 should be destroyed");
        assert!(app.world().get::<Station>(station2).is_none(), "Station 2 should be destroyed");
        assert!(app.world().get::<Fleet>(fleet).is_none(), "Fleet should be destroyed");
        assert!(app.world().get::<Station>(station3).is_some(), "Station 3 should survive");
    }
    ...
    fn test_kessler_gambit_creates_debris_field() {
        ...
        app.world_mut().spawn((Station { station_type: StationType::Outpost }, InOrbit { parent: planet }));
        app.world_mut().spawn((Station { station_type: StationType::MiningPlatform }, InOrbit { parent: planet }));
        ...
        assert!(debris.0 > 100.0, "Debris density should skyrocket after triggering the gambit");
    }
}