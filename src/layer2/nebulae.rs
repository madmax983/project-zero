...
pub fn nebula_effects_system(time: Option<Res<Time>>,
    ...
    for (fleet_transform, maybe_sensors, maybe_shields, maybe_speed, maybe_hull) in fleet_query.iter_mut() {
        ...
        for (nebula, volume, nebula_transform) in nebula_query.iter() {
            let distance = fleet_transform.translation.distance(nebula_transform.translation);
