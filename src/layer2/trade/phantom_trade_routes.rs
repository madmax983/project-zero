    ...
    if let Some(ref mut res) = resources {
        ...
        if food_to_consume > 0.0 && res.try_consume(ResourceType::Food, food_to_consume) {
            for mut morale in morale_query.iter_mut() {
                morale
                    .modifiers
                    .push(crate::layer1::social::morale::MoodModifier {
                        value: 0.05,
                        duration: 100,
...
impl bevy_app::Plugin for PhantomTradeRoutePlugin {
    fn build(&self, app: &mut bevy_app::App) {
        app.add_event::<FixPhantomRouteEvent>().add_systems(
            bevy_app::Update,
            (
                process_phantom_trade_routes_system,
                fix_phantom_route_reaction_system,
            ),
        );
    }
}
...
mod tests {
    ...
    fn test_phantom_trade_route_consumes_resources_and_boosts_morale() {
        ...
        app.world_mut().spawn(PhantomTradeRoute {
            required_resource: ResourceType::Food,
            amount: 10.0,
        });
        ...
        assert_eq!(
            current_resources.food, 90.0,
            "Phantom route should consume resources"
        );

        // Check morale boosted
        let morale = app.world().get::<Morale>(pop_id).unwrap();
        let has_boost = morale
            .modifiers
            .iter()
            .any(|m| m.value > 0.0 && m.label == "Purpose: Fulfilling Quota");
        assert!(
            has_boost,
            "Pop should gain morale from fulfilling the phantom quota"
        );
    }
    ...
    fn test_fixing_phantom_trade_route_shatters_morale() {
        ...
        app.world_mut()
            .send_event(FixPhantomRouteEvent { entity: route_id });
        ...
        // Check route removed
        assert!(
            app.world().get::<PhantomTradeRoute>(route_id).is_none(),
        ...
        let has_penalty = morale
            .modifiers
            .iter()
            .any(|m| m.value < 0.0 && m.label == "Despair: Pointless Labor");
        assert!(
            has_penalty,
            "Fixing the route should cause a severe morale penalty"
        );
    }
}
