...
use crate::layer2::fleet::{FleetCommand, WorldEater};
...
mod tests {
    ...
    fn test_world_eater_converts_tiles_to_bedrock() {
        ...
        assert_eq!(
            grid.get(5, 5).unwrap(),
            TerrainType::Void,
            "World Eater should convert the target tile to Bedrock(Void)."
        );

        let resources = app.resource::<RawResources>();
        assert_eq!(
            resources.amount, 100,
            "World Eater should generate resources based on efficiency."
        );
    }
}
