...
mod tests {
    ...
    fn test_invading_primitive_civilization_spawns_slave_pops() {
        ...
        events.send(InvasionEvent { target: planet, aggressor: FactionId::Unaligned });
        ...
        let slave_count = app.world_mut().query_filtered::<&Pop, With<Slave>>().iter(app.world()).count();
