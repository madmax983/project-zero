...
/// Unique identifier for a faction (guild).
...
pub enum FactionId {
    ...
    /// Guild for farmers.
    FarmersGuild,
    /// Guild for builders and masons.
    MasonsGuild,
    /// Guild for loggers and forestry workers.
    LoggersGuild,
    /// Guild for crafters and artisans.
    ArtisansGuild,
    /// Pops with no dominant skill or balanced skills.
    Unaligned,
}

/// Dynamic data for a faction.
#[derive(Debug, Clone)]
pub struct FactionData {
    /// Display name of the faction.
    pub name: String,
    /// Collective satisfaction (0.0 to 1.0).
...
/// Resource storing all active factions.
...
/// Component tagging a Pop as a member of a faction.
...
///
/// Runs when `Skills` component changes.
...
/// System to update faction member counts and satisfaction.
...
#[cfg(test)]
mod tests {
    use super::*; // Use logic from above
    use crate::layer1::edicts::{ColonyPolicies, Policy};
    ...
    fn test_policy_affects_satisfaction() {
        ...
        let factions = world.resource::<Factions>();
        // We expect satisfaction to decrease because we enabled a policy
        ...
        if let Some(miners) = factions.get(FactionId::MinersGuild) {
            ...
            assert!(
                factions.get(FactionId::MinersGuild).is_some(),
                "Factions should be initialized"
            );
        }
    }
}
