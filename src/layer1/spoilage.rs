...
/// Component indicating an entity (usually a dropped `ResourceItem`) decays over time.
#[derive(Component, Debug, Clone, Copy)]
pub struct Perishable {
    /// How many ticks the item has existed.
    pub current_ticks: u32,
    /// When `current_ticks` >= `max_ticks`, the entity is destroyed.
    pub max_ticks: u32,
}
...
/// Global spoilage rate for food in `ColonyResources` (0.05% per tick).
pub const GLOBAL_SPOILAGE_RATE: f32 = 0.0005;

/// System handling decay of global food stocks and perishable items.
...
pub fn spoilage_system(
    ...
    if let Some(ref mut resources) = resources {
