...
pub fn update_biometric_drift_system(
    mut profiles: Query<(&Age, &Health, &mut BiometricProfile)>,
) {
...
mod tests {
    ...
    fn test_biometric_drift_denies_access() {
        ...
        let pop = app.world_mut().spawn((
            Age { ticks_alive: 3000, ..Default::default() },
            Health { current: 50.0, max: 100.0, has_rust_lung: false }, // Missing 50 health
            BiometricProfile { baseline_age_ticks: 2000, drift_value: 0.0 },
            DoorAccessRequest { target: Entity::PLACEHOLDER, granted: true }, // Start true to see if it flips
        )).id();
        ...
        schedule.add_systems((update_biometric_drift_system, check_door_access_system.after(update_biometric_drift_system)));
    ...
    fn test_recalibration_restores_access() {
        ...
        let pop = app.world_mut().spawn((
            Age { ticks_alive: 3000, ..Default::default() },
            BiometricProfile { baseline_age_ticks: 2000, drift_value: 15.0 },
        ...
        assert_eq!(profile.baseline_age_ticks, 3000, "Baseline age should be updated");
