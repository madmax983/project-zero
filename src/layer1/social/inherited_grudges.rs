...
pub fn inherit_vendettas_system(
    ...
    mut param_set: ParamSet<(Query<&Vendettas>, Query<&mut Vendettas>)>,
) {
    for event in events.read() {
        let mut parent_targets = None;
        if let Ok(parent_vendettas) = param_set.p0().get(event.parent) {
            parent_targets = Some(parent_vendettas.targets.clone());
        }

        if let Some(targets) = parent_targets {
            if let Ok(mut child_vendettas) = param_set.p1().get_mut(event.child) {
