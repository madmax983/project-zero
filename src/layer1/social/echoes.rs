...
pub fn echo_aura_system(
    ...
    for (pop_pos, mut skills, mut stress) in pops.iter_mut() {
        for (echo, echo_pos) in echoes.iter() {
            ...
            if dist <= echo.radius {
                skills.xp.entry(SkillType::Engineering).and_modify(|x| *x += 10.0).or_insert(10.0);
...
mod tests {
    ...
    fn test_echo_spawns_from_source() {
        ...
        let _source = world.spawn((EchoSource { spawn_chance: 1.0 }, GridPosition { x: 5, y: 5 })).id();
    ...
    fn test_pop_near_echo_gains_bonus_xp_and_stress() {
        let mut world = World::new();
        // Spawn an Echo
        world.spawn((Echo { radius: 2.0, timer: Timer::from_seconds(60.0, TimerMode::Once) }, GridPosition { x: 5, y: 5 }));
        ...
        assert!(skills.get_xp(SkillType::Engineering) > 0.0, "Pop should gain XP from being near the Echo.");
    ...
    fn test_pop_far_from_echo_unaffected() {
        ...
        assert_eq!(skills.get_xp(SkillType::Engineering), 0.0, "Pop far away should not gain XP.");
    ...
    fn test_echo_despawns_after_timer() {
        ...
        let echo_entity = app.world_mut().spawn((
            Echo {
                radius: 3.0,
        ...
        app.world_mut().run_system_once(echo_despawn_system).unwrap();



        assert!(app.world().get_entity(echo_entity).is_err(), "Echo should have despawned after timer finished.");
    }
}
