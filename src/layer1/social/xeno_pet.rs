...
pub fn xeno_pet_reproduction_system(
    ...
    for mut pet in query.iter_mut() {
        if pet.reproduction_progress >= 90.0 && resources.try_consume(ResourceType::Food, 10.0) {
            ...
            commands.spawn((
                XenoPet { hunger: 0.0, reproduction_progress: 0.0 },
                ProtectedPet
            ));
        }
    }
...
mod tests {
    ...
    fn test_xeno_pet_increases_morale() {
        ...
        let entity = world.spawn(Morale { value: 0.5, modifiers: vec![] }).id();
        ...
        assert!(morale.value > 0.5, "Morale should increase when owning a Xeno-Pet");
    ...
    fn test_xeno_pet_reproduction_consumes_food() {
        ...
        world.spawn(XenoPet { hunger: 0.0, reproduction_progress: 90.0 });
        ...
        assert!(resources.food < 100.0, "Food should be consumed during reproduction");
        ...
        assert_eq!(protected_count, 1, "The newly spawned pet should be protected");
    }
}
