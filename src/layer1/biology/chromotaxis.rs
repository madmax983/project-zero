...
mod tests {
    ...
    fn test_chromotaxis_attraction() {
        ...
        let _building_id = app.world_mut().spawn((
            Building::default(),
            StructuralColor::Red,
        ...
        let fauna_id = app.world_mut().spawn((
            Fauna { fauna_type: FaunaType::Wolf, state: Default::default(), target: None, detection_range: 10.0, attack_cooldown: 0 },
        ...
        let fauna_velocity = app.world().entity(fauna_id).get::<ChromotaxisVelocity>().unwrap();
    ...
    fn test_chromotaxis_repulsion_aggro() {
        ...
        let _fauna_id = app.world_mut().spawn((
            Fauna { fauna_type: FaunaType::Wolf, state: Default::default(), target: None, detection_range: 10.0, attack_cooldown: 0 },
            ChromotaxisTrait { attractive_color: StructuralColor::Red, repulsive_color: StructuralColor::Blue },
        ...
        assert!(building_health.current < 100.0, "Building should take damage from aggro fauna due to repulsive color.");
    }
    ...
    fn test_chromotaxis_neutral_color() {
        ...
        let building_health = app.world().entity(building_id).get::<Health>().unwrap();
        assert_eq!(building_health.current, 100.0, "Building should not take damage from neutral color.");
    }
}
