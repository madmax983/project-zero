...
pub fn cybernetic_mind_merge_system(
    ...
    for (mut weights, integration) in query.iter_mut() {
        ...
        weights.distance_weight =
            weights.distance_weight * (1.0 - blend) + avg.distance_weight * blend;
        weights.availability_weight =
            weights.availability_weight * (1.0 - blend) + avg.availability_weight * blend;
    }
}
...
mod tests {
    ...
    fn test_cybernetic_integration_removes_biological_needs() {
        ...
        assert_eq!(
            needs.hunger, 1.0,
            "Integrated Pop should have Hunger frozen at 1.0"
        );
        assert_eq!(
            needs.rest, 1.0,
            "Integrated Pop should have Rest frozen at 1.0"
        );
    }
