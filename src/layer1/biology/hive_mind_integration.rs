...
pub fn process_integration_surgery_system(
    ...
    mut query: Query<&mut Traits, With<crate::layer1::pop::Pop>>,
) {
    for event in events.read() {
        if event.procedure == "XenoIntegration" {
            ...
            // Wipe personality by removing all elements from the HashSet
...
// System to freeze needs
pub fn freeze_collective_needs_system(
    mut query: Query<&mut Needs, With<IntegratedCollective>>
) {
...
mod tests {
    ...
    fn test_xeno_integration_removes_needs_and_traits() {
        ...
        app.add_systems(Update, (process_integration_surgery_system, freeze_collective_needs_system).chain());
        ...
        assert_eq!(needs.rest, 1.0, "Integrated pops should not need sleep.");
        assert_eq!(needs.leisure, 1.0, "Integrated pops should not need leisure.");

        let traits = app.world().get::<Traits>(pop).unwrap();
        assert!(traits.0.is_empty(), "Integrated pops should lose individual traits.");
    }
    ...
    fn test_collective_evaluates_unintegrated_pops_as_inefficient() {
        ...
        let unintegrated = app.world_mut().spawn((Pop, Needs { hunger: 0.0, rest: 1.0, leisure: 1.0, hygiene: 1.0 })).id();
        let another_integrated = app.world_mut().spawn((Pop, IntegratedCollective, Needs { hunger: 0.0, rest: 1.0, leisure: 1.0, hygiene: 1.0 })).id();
