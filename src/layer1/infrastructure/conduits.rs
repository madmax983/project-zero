...
pub fn tap_conduit_power_system(
    mut conduits: Query<(&ExposedConduit, &mut PowerSource)>,
) {
    for (conduit, mut power) in conduits.iter_mut() {
        if conduit.resource_type == ConduitResource::Power {
            power.output = conduit.yield_amount;
            power.active = true;
        }
    }
}
...
mod tests {
    ...
    fn test_mining_reveals_ancient_conduit() {
        ...
        let miner = app.world_mut().spawn(()).id();

        // Mine the tile containing the buried conduit
        app.world_mut().send_event(MineEvent { miner, target: entity });
        app.update();
