...
pub fn handle_vent_pathfinding(
    ...
    // REFACTOR Phase: O(1) adjacency lookup
    let mut connection_map = bevy::utils::HashMap::new();
    for vent in vents.iter() {
        connection_map.insert((vent.pos_a, vent.pos_b), vent.grated);
        connection_map.insert((vent.pos_b, vent.pos_a), vent.grated);
    }
    ...
    for event in events.read() {
        ...
        if is_small {
            // Check if there is an ungrated vent connecting start and end in O(1)
            if let Some(grated) = connection_map.get(&(event.start, event.end)) {
                if !grated {
                    success = true;
                }
            }
...
mod tests {
    ...
    fn test_small_entities_can_traverse_vents() {
        ...
        let _vent = app.world_mut().spawn(VentConnection { pos_a, pos_b, grated: false }).id();
