...
/// Component for machines that emit vibration.
#[derive(Component)]
...
/// Component for fragile items that react to vibration.
#[derive(Component)]
pub struct ResonantCrystal {
    pub fragility_threshold: u32,
}

/// Event emitted when heavy machinery is active.
#[derive(Event)]
...
pub fn process_vibration_resonance_system(
    ...
    mut crystal_query: Query<(&GridPosition, &ResonantCrystal, &mut crate::layer1::biology::Health)>,
) {
    for event in vibration_events.read() {
        for (pos, crystal, mut health) in crystal_query.iter_mut() {
            ...
            let distance = (pos.x - event.origin.x).abs()
                         + (pos.y - event.origin.y).abs();
...
mod tests {
    ...
    fn test_heavy_machinery_emits_vibration() {
        ...
        let mut reader = vibration_events.get_cursor();
    ...
    fn test_vibration_shatters_crystals() {
        ...
        app.world_mut().send_event(VibrationEvent {
