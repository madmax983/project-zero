...
fn find_structure_target(
    ...
    for proxy in &buffer.all_structures {
        if proxy.entity == data.entity {
            continue;
        }
        let dist = manhattan_distance(&data.pos, &proxy.pos);
        if dist < closest_dist {
            closest_dist = dist;
            closest_target = Some(proxy.entity);
        }
    }
...
fn find_food_target(
    ...
    for proxy in &buffer.stockpiles {
        let dist = manhattan_distance(&data.pos, &proxy.pos);
        if dist < closest_dist {
            closest_dist = dist;
            closest_target = Some(proxy.entity);
