...
pub fn calculate_average_utility_system(
...
mod tests {
    ...
    #[test]
    fn test_calculate_average_utility() {
        let mut app = App::new();
        app.init_resource::<ColonyAverageUtility>();
        app.add_systems(Update, calculate_average_utility_system);
        ...
        assert!((avg.weights.distance_weight - 0.5).abs() < f32::EPSILON);
        assert!((avg.weights.availability_weight - 0.5).abs() < f32::EPSILON);
    }
}
