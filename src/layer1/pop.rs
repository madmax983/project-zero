...
pub fn spawn_initial_pops(world: &mut World) {
    ...
    // Safety valve to prevent infinite loops if map is bad
    let mut attempts = 0;
    let max_attempts = 1000;

    // Get dimensions first to avoid holding borrow
    let (width, height) = {
    ...
    while spawned < 5 && attempts < max_attempts {
        ...
        // Casts are safe assuming reasonable terrain dimensions (< i32::MAX)
        #[allow(clippy::cast_possible_truncation, clippy::cast_possible_wrap)]
        let x = rng.gen_range(0..width as i32);
        #[allow(clippy::cast_possible_truncation, clippy::cast_possible_wrap)]
        let y = rng.gen_range(0..height as i32);

        // Check if walkable - scope the borrow so we can spawn later
        let is_walkable = {
            let terrain = world.resource::<TerrainGrid>();
            // x/y are generated from positive range
            #[allow(clippy::cast_sign_loss)]
            terrain.get(x as usize, y as usize).is_some_and(|terrain_type| {
                terrain_type != TerrainType::Water && terrain_type != TerrainType::Rock
            })
        };
