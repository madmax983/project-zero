...
mod tests {
    ...
    fn test_zero_g_flora_growth_requires_microgravity() {
        ...
        let mut growth_conditions = ZeroGGrowthConditions::default();
    ...
    fn test_zero_g_flora_yields_high_value_resource() {
        ...
        let crop = ZeroGCrop {
            ...
            conditions: ZeroGGrowthConditions { requires_microgravity: true },
            yield_resource: ResourceType::HyperValuable,
        ...
        // Assert
        assert!(matches!(resource_yield.resource_type, ResourceType::HyperValuable));
    ...
    fn test_micro_meteorite_strike_destroys_crop() {
        ...
        let farm = app.world_mut().spawn((
            ...
            Station { station_type: StationType::Hydroponics }
        )).id();
        ...
        assert!(crop_yield.is_none() || crop_yield.unwrap().amount == 0.0, "Crops must be destroyed upon depressurization.");

        let events = app.world().resource::<Events<DepressurizationEvent>>();
        assert!(!events.is_empty(), "DepressurizationEvent should be emitted");
    }
}
