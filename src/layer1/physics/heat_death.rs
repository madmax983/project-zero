...
pub fn overheat_scram_system(mut query: Query<(&mut Operational, &Temperature, &ScramThreshold)>) {
...
mod tests {
    ...
    fn test_heat_accumulation_in_vacuum() {
        ...
        assert_eq!(
            app.world().get::<Temperature>(machine).unwrap().current,
            30.0
        );
    }
    ...
    fn test_radiator_dissipates_heat() {
        ...
        app.add_systems(
            Update,
            (heat_accumulation_system, radiator_cooling_system).chain(),
        );
        ...
        app.world_mut()
            .entity_mut(machine)
            .insert(ConnectedRadiator(radiator));
    ...
    fn test_machine_scrams_on_overheat() {
        ...
        assert!(!app.world().get::<Operational>(machine).unwrap().0);
    }
}
