...
pub fn has_line_of_sight(world: &World, observer: Entity, target: Entity) -> bool {
    ...
    let observer_pos = world.get::<crate::layer1::core::map::GridPosition>(observer).unwrap();
    let observer_elev = world.get::<Elevation>(observer).map(|e| e.0).unwrap_or(0.0);

    let target_pos = world.get::<crate::layer1::core::map::GridPosition>(target).unwrap();
...
mod tests {
    ...
    fn test_planetary_curvature_limits_los() {
        ...
        let observer = app.world_mut().spawn((crate::layer1::core::map::GridPosition { x: 0, y: 0 }, Elevation(0.0))).id();
        let target = app.world_mut().spawn((crate::layer1::core::map::GridPosition { x: 15, y: 0 }, Elevation(0.0))).id();
    ...
    fn test_elevation_extends_horizon() {
        ...
        let observer = app.world_mut().spawn((crate::layer1::core::map::GridPosition { x: 0, y: 0 }, Elevation(10.0))).id();
        let target = app.world_mut().spawn((crate::layer1::core::map::GridPosition { x: 15, y: 0 }, Elevation(0.0))).id();
