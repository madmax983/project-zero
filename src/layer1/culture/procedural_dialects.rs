...
pub fn decay_slang_weight(
    ...
    // Decay logic based on time ticks
...
mod tests {
    ...
    fn test_event_creates_dialect_slang() {
        ...
        app.world_mut().send_event(AddChronicleEvent {
            text: "A massive Fire destroyed the farm".to_string(),
    ...
    fn test_slang_decays_over_time_without_reinforcement() {
        let mut app = App::new();
        app.init_resource::<DialectManager>();
        app.insert_resource(crate::shared::time::SimulationTime { tick: 100, speed: crate::shared::time::SimSpeed::Normal });
