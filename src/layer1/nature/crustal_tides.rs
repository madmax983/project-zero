...
use crate::layer2::syzygy::TidalForce; // We will create this or mock it
use crate::layer1::map::GridPosition;

pub fn process_crustal_tides(
    mut terrain: ResMut<TerrainGrid>,
    tidal_force: Option<Res<TidalForce>>,
    mut buildings: Query<(&GridPosition, &mut Health)>,
) {
    let strength = if let Some(t) = tidal_force { t.current } else { 0.5 };
    let high_tide = strength > 0.7;
    let low_tide = strength < 0.3;
