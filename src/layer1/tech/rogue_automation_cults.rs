...
pub fn rogue_cult_formation_system(
    ...
    let potential_shrine = shrines_query.iter().find(|(_, b_type, debt)| {
        (b_type.building_type == BuildingType::Mainframe
            || b_type.building_type == BuildingType::CommsRelay)
...
pub fn cult_priority_override_system(mut query: Query<(&MachineCultMember, &mut UtilityWeights)>) {
    ...
    fn test_cult_membership_overrides_standard_utility_weights() {
        ...
        let shrine = app
            ...
            .spawn((
                ...
                crate::layer1::tech::rogue_automation_cults::MaintenanceDebt {
                    current: 150.0,
                    threshold: 100.0,
                },
            ))
        ...
        assert!(
            weights.work < 0.5,
            "Standard work weight should be suppressed"
        );
        assert!(
            weights.defend > 0.8,
            "Defend weight should spike to protect the shrine"
        );
    }
}
