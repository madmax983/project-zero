...
mod tests {
    ...
    fn test_ghost_trait_legacy_targeting() {
        ...
        let turret_entity = world.spawn((
            Building { building_type: BuildingType::Tower }, // Use Tower or TrashCannon as they are turrets
            Turret {
        ...
        assert!(turret.attack.range > initial_range, "Range should be increased by LegacyTargeting");
        assert!(turret.attack.damage > initial_damage, "Damage should be increased by LegacyTargeting");

        // Specific values (1.5x range, 1.2x damage based on plan)
        assert!((turret.attack.range - (initial_range * 1.5)).abs() < f32::EPSILON, "Range should be 1.5x");
        assert!((turret.attack.damage - (initial_damage * 1.2)).abs() < f32::EPSILON, "Damage should be 1.2x");
    }
}
