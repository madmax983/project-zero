...
pub fn consume_rations_system(
    ...
    let penalty = config
        .map(|c| c.biomass_taint_stress_penalty)
        .unwrap_or(50.0);
...
mod tests {
    ...
    fn test_corpse_recycling_produces_tainted_biomass() {
        ...
        assert!(
            query[0].has_grief_taint,
            "Biomass produced from a Corpse MUST have the grief taint"
        );
    }
    ...
    fn test_consuming_tainted_biomass_increases_stress() {
        ...
        let soul = world
            .spawn((
                ...
                DigestionQueue(vec![BiomassRation {
                    has_grief_taint: true,
        ...
        assert!(
            ...
            "Consuming tainted biomass must increase the Soul's Stress"
        );
    }
    ...
    fn test_recycling_multiple_events_same_corpse() {
        ...
        assert_eq!(
            query.len(),
            1,
            "Should produce only one batch of Biomass despite duplicate events"
        );
    }
}
