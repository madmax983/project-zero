...
mod tests {
    ...
    fn test_wearing_bio_suit_provides_armor() {
        ...
        let mut app = App::new();
        let suit = app.world_mut().spawn(BioSuit { hunger: 0.0, attachment_level: 0.0, armor_applied: false }).id();
    ...
    fn test_bio_suit_hunger_drains_pop_health() {
        ...
        let mut app = App::new();
        let suit = app.world_mut().spawn(BioSuit { hunger: 80.0, attachment_level: 50.0, armor_applied: false }).id();
        let pop = app.world_mut().spawn((
            Pop,
            Equipment { body: Some(suit), ..Default::default() },
