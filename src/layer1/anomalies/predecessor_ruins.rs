...
pub fn predecessor_ruins_passive_bonus_system(
    ...
    for ruin in query.iter() {
        if !ruin.awakened {
            resources.add_knowledge(ruin.research_bonus_rate * time.delta_secs());
        }
    }
}
...
pub fn predecessor_ruins_awakening_system(
    ...
    energy_query: Query<&PowerSource>,
) {
    let mut total_grid_energy = 0.0;
    for source in energy_query.iter() {
        if source.active {
...
mod tests {
    ...
    fn test_predecessor_ruins_provide_passive_research_bonus() {
        ...
        app.insert_resource(ColonyResources { knowledge: 0.0, max_knowledge: 1000.0, ..Default::default() });
        app.world_mut().spawn(PredecessorRuin { awakened: false, research_bonus_rate: 5.0 });

        // Advance time manually if necessary, or just run the system.
        // We'll insert a simulated time delta.
    ...
    fn test_massive_energy_spike_awakens_dormant_systems() {
        ...
        app.world_mut().spawn(PowerSource { output: 10000.0, active: true });
        ...
        // Assert orbital shield exists
        let mut query = app.world_mut().query::<&PredecessorOrbitalShield>();
        let shield_count = query.iter(app.world()).count();
        assert_eq!(shield_count, 1, "Orbital shield should spawn upon awakening");
    }
}
