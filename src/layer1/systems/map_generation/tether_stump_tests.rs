...
use bevy::prelude::*; // We will use AtmosphereGrid since PressureGrid does not exist
...
fn test_tether_stump_generation() {
    ...
    assert!(
        stump_tiles > 0 && stump_tiles <= 9,
        "Stump should have a small footprint"
    );
}
...
fn test_atmosphere_pressure_drops_with_height() {
    ...
    assert!(
        high_altitude_pressure < ground_pressure,
        "Pressure should drop with altitude"
    );
    assert!(
        high_altitude_pressure < 0.5,
        "High altitude should be dangerously thin"
    );
}
...
fn test_lost_tech_spawns_at_high_altitude() {
    ...
    app.add_systems(
        Startup,
        (generate_tether_stump, spawn_lost_tech_caches).chain(),
    );
    ...
    for (transform, _) in tech_query.iter(app.world()) {
        assert!(
            ...
            "Lost tech should only spawn at high altitude"
        );
    }
}
