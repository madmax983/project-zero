...
*Builder: The RED Phase uses placeholder components (`PopAction` with an `ActionState` enum, `GridPosition` redefined locally, etc.) instead of the actual `PopAction` in `src/layer1/mind/utility_types.rs` which uses `ActionType`. The real `GridPosition` is in `src/layer1/map.rs`. Also, the actual game has a `ThermalBloom` component in `src/layer2/thermal.rs` and `DetectionRisk` system, but this spec seems aimed at Layer 1 ground combat/fauna mechanics. Should I implement this using exact standalone placeholders inside `src/layer1/environment/thermal_camouflage.rs` to pass the tests as written, or adapt it to the real `PopAction` and `GridPosition`? Because the prompt requires strict adherence to the spec's TDD flow, I will implement this with real `GridPosition` and adapt the `PopAction` placeholder into `ActionType` or just use the exact RED phase code if it's meant to be an isolated subsystem.*

*I will adapt the RED phase test to use `crate::layer1::map::GridPosition` and the real `crate::layer1::mind::utility_types::PopAction` and `ActionType` components if they exist, to avoid redefining core structs, but I'll add `ThermalSignature` and `ThermalSensor` as specified.*

*Architect:* Yes, please use the actual `GridPosition` and `ActionType` components. The intent was not to create an isolated placeholder module if real equivalents exist. Ensure the tests pass using the real component infrastructure.
