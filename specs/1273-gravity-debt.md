...
High-mass logistics slowly but inevitably warping the physical and political landscape of a system. Repeatedly launching massive freighters from a low-gravity world to a high-gravity world is cheap, but doing the reverse requires immense energy. Over centuries, heavily exploited mining worlds develop "Gravity Debt"—the sheer mass of exported material physically alters the planet's orbital mechanics, making subsequent launches exponentially more expensive and increasing the frequency of catastrophic launch failures. Your core world imports so much raw material from the fringe that it physically traps its own massive population, unable to afford the energy to launch emigration fleets when the resources run dry.

## Dependencies

- None explicitly, but relies on existing Layer 2 node tracking and logistics networks (e.g., freighters transferring mass).
...
mod tests {
    ...
    fn test_gravity_debt_accumulation() {
        ...
        let planet_entity = app.world_mut().spawn((
            ...
            GravityDebt { imported_mass: 0.0, exported_mass: 0.0 },
        )).id();

        app.world_mut().spawn(FreighterArrivalEvent {
            destination: planet_entity,
            mass_amount: 5000.0,
        });

        app.world_mut().spawn(FreighterDepartureEvent {
    ...
    fn test_launch_cost_scales_with_debt() {
        // Arrange
        let base_cost = 100.0;
        let mut planet_debt = GravityDebt { imported_mass: 100000.0, exported_mass: 0.0 };
    ...
    fn test_launch_failure_chance_increases() {
         let mut planet_debt = GravityDebt { imported_mass: 500000.0, exported_mass: 0.0 };
...
- **Refactoring:** The hardcoded `0.0001` modifier constant should be extracted to a tuning config or `Res<PhysicsConstants>`.
- **Code Smells:** Passing raw `f32` for mass is slightly error-prone. Consider a strongly typed `Mass(f32)` newtype to prevent accidental mixing with volume or cost.
- **Performance:** This system is event-driven and updates only when freighters move, which is highly performant. However, we may want to recalculate the actual planet `base_gravity` occasionally rather than dynamically deriving the modifier every time, if it starts being read frequently by UI.
...
- [ ] `GravityDebt` correctly increases launch costs and failure chances based on net mass changes.

## Technical Guidance

- Integration with existing logistics: Ensure whatever system spawns freighters fires the `FreighterArrivalEvent` and `FreighterDepartureEvent`.
- If a planet is destroyed or fundamentally altered, consider how the mass redistributes (e.g., turning into a debris field or asteroid belt).
