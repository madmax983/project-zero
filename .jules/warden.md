...
**2024-05-20 - [Dangling Transmute in Trait Iteration]**
**Threat:** [Undefined Behavior (UB)] `Traits::iter()` used `unsafe { std::mem::transmute }` over a hardcoded range (`0..27`). Since `Trait` now has 41 variants, transmuting outside this range (if the loop were updated without fixing the max bound) or modifying the enum without perfectly contiguous values could cause immediate UB or memory safety violations. In addition, there was a logic bug where variants >26 were silently dropped from iteration.
**Defense:** [Safe Enum Iteration] Removed the `unsafe` block entirely. Added `strum::EnumIter` to the `Trait` enum, and refactored `Traits::iter()` to map over `Trait::iter()`, relying purely on safe bitmask checks. This mathematically guarantees we only construct valid `Trait` variants and dynamically scales when new traits are added without hardcoded bounds.
