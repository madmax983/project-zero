...
**[Title] Break Up Blob Pattern in Core Integration - Cassandra Syndrome**
**Tangle:** The `src/layer1/core/integration.rs` module acted as a giant "Blob" file, bridging almost every domain together and creating massive coupling. Specifically, the `cassandra_syndrome` bridges (`cassandra_syndrome_chronicle_bridge` and `cassandra_cult_chronicle_bridge`) were located here, tightly coupling core integration to a specific anomaly domain.
**Blueprint:** Extracted the `cassandra_syndrome_chronicle_bridge` and `cassandra_cult_chronicle_bridge` functions from `src/layer1/core/integration.rs` and moved them directly into their domain module `src/layer1/cassandra_syndrome/mod.rs`. Updated the system registrations in `src/layer1/systems/observation.rs` and the integration tests in `tests/integration/cassandra_syndrome_bridge.rs` to pull from the correct, cohesive domain boundary.
