...
**2026-07-17 - Purged UI Formatting Dependency from Domain Logic**
**Tangle:** The `NarrativeError` domain error type explicitly constructed and returned a `comfy_table::Table` using the `comfy-table` crate. This bound core domain logic to a heavy terminal UI formatting dependency, leaking presentation concerns into the data layer and violating the KISS principle. Furthermore, the `src/prelude.rs` module incorrectly blocked the export of the `OralTradition` fallback stubs when the `nova` feature was disabled, preventing users from seeing the helpful `#[deprecated]` warnings.
**Blueprint:** Refactored `NarrativeError::to_table()` to format its output using standard `format!` macros, dropping the internal `comfy_table` usage completely from `src/shared/narrative.rs`. Removed the restrictive `#[cfg(feature = "nova")]` gates from the `src/prelude.rs` exports, allowing the compiler to reach the fallback definitions unconditionally.
