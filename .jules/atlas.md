...
**[Title] Break Dependency Between Core Narrative and UI Presentation**
**Tangle:** The core `NarrativeError` defined in `src/shared/narrative.rs` implemented a `to_table` method that directly returned a formatted `comfy_table::Table` (or a formatted String utilizing it). This tied the domain logic of narratives tightly to a terminal UI rendering crate (`comfy_table`), violating the rule against having heavy dependencies in core logic. It also leaked UI representation concerns to the consumer, making it awkward for non-CLI clients (like WASM) to use.
**Blueprint:** Removed `comfy_table` usage from `src/shared/narrative.rs` entirely. Consumers (such as `README.md` and examples) now use the derived `Display` (via `thiserror`) to render the error context via `println!("{}", e)`, leaving UI rendering choices to the outer layers.
