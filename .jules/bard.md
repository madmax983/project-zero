...
## 2026-07-14 - [Fixed Ancestral Graves Doctests and Formatting]
**Confusion:** The documentation in `src/layer1/culture/ancestral_graves.rs` caused compilation errors and warnings. HTML tags were used incorrectly instead of markdown backticks for struct names like `BuildingCompletedEvent`. Empty lines followed `///` doc comment block headers, violating clippy rules. Also, a broken intra-doc link for `BuildingCompletedEvent` caused cargo doc to warn.
**Clarification:** Replaced invalid HTML `<Event>` notation with proper backtick code blocks in docstrings. Removed trailing whitespace/empty lines within `///` comments to satisfy `clippy::empty_line_after_doc_comments`. Fully qualified the intra-doc link to `[`crate::layer1::core::events::BuildingCompletedEvent`]` to ensure docs build correctly.
