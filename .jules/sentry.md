...
## 2024-04-22 - [Coverage improvement in Execution systems]
**Learning:** Testing logic that acts on Bevy queries inside normal utility systems often requires `RunSystemOnce` if you want to avoid tightly coupling mock `Commands` or `World` resources outside the ECS, particularly for `arrival` and `demolish` interactions where components get despawned immediately. Setting defaults for `ColonyResources` in tests also requires verifying `max_*` limits since resources can easily clamp to 0 if not initialized properly.
**Action:** Extract nested sub-system queries into standard systems where possible to allow testing them simply with `world.run_system_once(my_system_fn)`. Ensure mock resources provide necessary defaults that prevent boundary conditions from early-exiting.
