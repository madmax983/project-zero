...
for filename in os.listdir(specs_dir):
    ...
    for i, line in enumerate(lines):
        if "builder:" in line.lower() and "add questions here" not in line.lower() and "add any questions here" not in line.lower():
            # Check the next few lines for an Architect response
            found_answer = False
            for j in range(1, min(10, len(lines) - i)):
                if "architect:" in lines[i+j].lower() or "architect :" in lines[i+j].lower():
                    found_answer = True
                    break
            if not found_answer:
