...
9. **Update src/layer1/social/mod.rs tests for proximity_social_system**
   - We need to init the `ConversationEvent` in `test_proximity_morale_buff`, `test_proximity_morale_debuff` in `src/layer1/social/mod.rs` so tests don't panic due to `EventWriter`.
   - Use patch:
     ```diff
     <<<<<<< SEARCH
...
10. **Verify test edits**
    - Run `cat src/layer1/social/mod.rs | grep -A 5 "fn test_proximity_morale_buff"` and `cat src/layer1/social/mod.rs | grep -A 5 "fn test_proximity_morale_debuff"`.
11. **Run full tests**
